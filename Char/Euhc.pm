...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Euhc::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    @Char::Euhc::m_matched = (qr/(?{Char::Euhc::m_matched})/);
    @Char::Euhc::s_matched = (qr/(?{Char::Euhc::s_matched})/);
}
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Euhc::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
=item File test operator -X
  ...
  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Euhc::s(*), Char::Euhc::s_()   File has nonzero size (returns size)
