...
${Char::Euhc::anchor}      = qr{\G(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE])*?}oxms;
BEGIN { CORE::eval q{ use vars qw(
...
if (($] >= 5.010001) or
    ...
    # other encoding
        ...
        ${Char::Euhc::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[$sbcs](?:$tbcs_1st[^$sbcs]{2})*?}oxms;
    }

    ${Char::Euhc::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE])*?|(?(?=[$sbcs]+\z).*?|(?:${Char::Euhc::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
...
#                                                                                      (?(?=[$sbcs]+\z) # (6)
#                                                                                                      .*?| #(7)
#                                                                                                          (?:${Char::Euhc::q_char_SADAHIRO_Tomoyuki_2002_01_17}) # (8)
#                                                                                                                                                           ))}oxms;

    # avoid: Complex regular subexpression recursion limit (32766) exceeded at here.
    local $^W = 0;

    if (((('A' x 32768).'B') !~ / ${Char::Euhc::anchor}                              B /oxms) and
        ((('A' x 32768).'B') =~ / ${Char::Euhc::anchor_SADAHIRO_Tomoyuki_2002_01_17} B /oxms)
    ) {
        ${Char::Euhc::anchor} = ${Char::Euhc::anchor_SADAHIRO_Tomoyuki_2002_01_17};
    }
    else {
        undef ${Char::Euhc::q_char_SADAHIRO_Tomoyuki_2002_01_17};
    }
}
...
# avoid: Name "Char::Euhc::foo" used only once: possible typo at here.
${Char::Euhc::dot}         = ${Char::Euhc::dot};
${Char::Euhc::dot_s}       = ${Char::Euhc::dot_s};
${Char::Euhc::eD}          = ${Char::Euhc::eD};
${Char::Euhc::eS}          = ${Char::Euhc::eS};
${Char::Euhc::eW}          = ${Char::Euhc::eW};
${Char::Euhc::eH}          = ${Char::Euhc::eH};
${Char::Euhc::eV}          = ${Char::Euhc::eV};
${Char::Euhc::eR}          = ${Char::Euhc::eR};
${Char::Euhc::eN}          = ${Char::Euhc::eN};
${Char::Euhc::not_alnum}   = ${Char::Euhc::not_alnum};
${Char::Euhc::not_alpha}   = ${Char::Euhc::not_alpha};
${Char::Euhc::not_ascii}   = ${Char::Euhc::not_ascii};
${Char::Euhc::not_blank}   = ${Char::Euhc::not_blank};
${Char::Euhc::not_cntrl}   = ${Char::Euhc::not_cntrl};
${Char::Euhc::not_digit}   = ${Char::Euhc::not_digit};
${Char::Euhc::not_graph}   = ${Char::Euhc::not_graph};
${Char::Euhc::not_lower}   = ${Char::Euhc::not_lower};
${Char::Euhc::not_lower_i} = ${Char::Euhc::not_lower_i};
${Char::Euhc::not_print}   = ${Char::Euhc::not_print};
${Char::Euhc::not_punct}   = ${Char::Euhc::not_punct};
${Char::Euhc::not_space}   = ${Char::Euhc::not_space};
${Char::Euhc::not_upper}   = ${Char::Euhc::not_upper};
${Char::Euhc::not_upper_i} = ${Char::Euhc::not_upper_i};
${Char::Euhc::not_word}    = ${Char::Euhc::not_word};
${Char::Euhc::not_xdigit}  = ${Char::Euhc::not_xdigit};
${Char::Euhc::eb}          = ${Char::Euhc::eb};
${Char::Euhc::eB}          = ${Char::Euhc::eB};
...
sub Char::Euhc::split(;$$$) {
    ...
    if (CORE::length($string) > 32766) {
        ...
        elsif (defined ${Char::Euhc::q_char_SADAHIRO_Tomoyuki_2002_01_17}) {
            $q_char = ${Char::Euhc::q_char_SADAHIRO_Tomoyuki_2002_01_17};
        }
    }
...
sub Char::Euhc::filetest {
    ...
    unless (CORE::eval qq{Char::Euhc::$filetest(\$file)}) {
        return '';
    }
...
sub Char::Euhc::filetest_ {
    ...
    unless (CORE::eval qq{Char::Euhc::${filetest}_}) {
        return '';
    }
    for my $filetest (CORE::reverse @_) {
        unless (CORE::eval qq{ $filetest _ }) {
...
sub my_home {
    ...
    if (defined $home and ! Char::Euhc::d($home)) {
        $home = undef;
    }
    return $home;
}
...
sub Char::UHC::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Char::UHC::escape_token();
    }
...
# subroutines of package Char::Euhc
    ...
    elsif (/\G \b Char::UHC::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::UHC::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Char::UHC::escape'; }
...
# subroutines of package Char::Euhc
        ...
        elsif ($string =~ /\G \b Char::UHC::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::UHC::eval \b                                /oxgc) { $e_string .= 'eval Char::UHC::escape'; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                                              2   3                                 4   5
                q<CORE::eval{local $Char::UHC::re_t=%s; local $Char::UHC::re_a=''; while($Char::UHC::re_t =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s$Char::UHC::re_t="$Char::UHC::re_a${1}$Char::UHC::re_r$'"; pos($Char::UHC::re_t)=length "$Char::UHC::re_a${1}$Char::UHC::re_r"; $Char::UHC::re_a=substr($Char::UHC::re_t,0,pos($Char::UHC::re_t)); } return $Char::UHC::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::UHC::re_r=CORE::eval $Char::UHC::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Char::UHC::re_t=%s; while($Char::UHC::re_t =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s$Char::UHC::re_t="$`$Char::UHC::re_r$'"; pos($Char::UHC::re_t)=length "$`$Char::UHC::re_r"; } return $Char::UHC::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Char::UHC::re_r=%s; %s"%s$Char::UHC::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $Char::UHC::re_n=0; local $Char::UHC::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="$Char::UHC::re_a${1}$Char::UHC::re_r$'"; pos(%s)=length "$Char::UHC::re_a${1}$Char::UHC::re_r"; $Char::UHC::re_a=substr(%s,0,pos(%s)); $Char::UHC::re_n++} return %s$Char::UHC::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Char::UHC::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="$`$Char::UHC::re_r$'"; pos(%s)=length "$`$Char::UHC::re_r"; $Char::UHC::re_n++} return %s$Char::UHC::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="%s$Char::UHC::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="%s$Char::UHC::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::UHC::re_r=CORE::eval $Char::UHC::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
This module has run-time routines for use Char::UHC software automatically, you
do not have to use.
...
=item Chop string
  ...
  character chopped. The Char::Euhc::chop subroutine is used primary to remove the newline
...
=item Reverse index string
  ...
  This subroutine works just like Char::Euhc::index except that it returns the byte
...
=item Lower case string
  ...
  You can use the Char::Euhc::fc subroutine for case-insensitive comparisons via Char::UHC
  software.
...
=item Fold case string
  ...
  New to Char::UHC software, this subroutine returns the full Unicode-like casefold of
...
=item File test subroutine Char::Euhc::X
...
=item Filename expansion (globbing)
  ...
  The Char::Euhc::glob subroutine grandfathers the use of whitespace to separate multiple
...
=item Statistics about file
  ...
  the last Char::Euhc::stat, Char::Euhc::lstat, or Char::Euhc::stat-based file test subroutine
