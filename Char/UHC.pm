...
######################################################################
#
# Char::UHC - Source code filter to UHC script
#
...
sub Char::UHC::escape_script {
    ...
    if (/^ use Char::Euhc(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Char::Euhc %s;\n%s", $Char::Euhc::VERSION, $use_re_eval); # require run-time routines version
...
Char::UHC - Source code filter to escape UHC

=head1 SYNOPSIS

  use Char::UHC;
  use Char::UHC version;         --- require version
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  m?regexp?               m?ditto@Char::Euhc::matched?
  s/regexp/replacement/   ($_ =~ m/ditto@Char::Euhc::matched/) ?
                          eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  m?regexp?b              m?regexp@Char::Euhc::matched?
  s/regexp/replacement/b  ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)@Char::Euhc::matched/) ?
                          eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
