...
######################################################################
#
# Char::UHC - Source code filter to escape UHC script
#
...
my $qq_substr  = qr{(?: Char::UHC::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $matched     = '';
my $s_matched   = '';
$matched        = q{$Char::Euhc::matched};
$s_matched      = q{ Char::Euhc::s_matched();};
...
my $function_ord;         # ord()   to ord()   or Char::UHC::ord()
my $function_ord_;        # ord     to ord     or Char::UHC::ord_
my $function_reverse;     # reverse to reverse or Char::UHC::reverse
my $function_getc;        # getc    to getc    or Char::UHC::getc
...
END
    ...
    print Char::UHC::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Char::Euhc::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            Char::Euhc::unlink "$filename.e";
        }
        elsif (Char::Euhc::z("$filename.e")) {
            Char::Euhc::unlink "$filename.e";
        }
        else {
            ...
            my $mtime_filter = (Char::Euhc::stat(__FILE__     ))[9];
            my $mtime_source = (Char::Euhc::stat($filename    ))[9];
            my $mtime_escape = (Char::Euhc::stat("$filename.e"))[9];
    ...
    if (not Char::Euhc::e("$filename.e")) {
        my $fh = gensym();

        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
        }
        else {
            Char::Euhc::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::UHC::escape_script($filename);
        print {$fh} $e_script;

        my $mode = (Char::Euhc::stat($filename))[2] & 0777;
    ...
    my $fh = gensym();
    Char::Euhc::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::UHC::escape_script {
    ...
    # read UHC script
    my $fh = gensym();
    Char::Euhc::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Euhc(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        if (s/^ \s* use \s+ Char::UHC \s* ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::UHC::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/UHC.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/UHC.pm $version, but @{[__FILE__]} is version $Char::UHC::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Euhc.pm $version, but \\\$Char::Euhc::VERSION is \$Char::Euhc::VERSION" if \$Char::Euhc::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::UHC::VERSION) {
                    die "Script $0 required Char/UHC.pm $version, but @{[__FILE__]} is only version $Char::UHC::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::UHC::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::UHC::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::UHC::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::UHC::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Euhc::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Euhc::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Euhc::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Euhc::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Euhc::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Euhc::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Char::Euhc::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Euhc::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Euhc::glob';
    }
...
# subroutines of package Char::Euhc
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                    }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Euhc::chop';         }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';               }
    elsif (/\G \b Char::UHC::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::UHC::index';         }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Euhc::index';        }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (/\G \b Char::UHC::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::UHC::rindex';        }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Euhc::rindex';       }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Euhc::lc';           }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Euhc::lcfirst';      }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Euhc::uc';           }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Euhc::ucfirst';      }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Euhc::fc';           }
    ...
    # (and so on)
                                                         ...
                                                         \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\2) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\'|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\2|\\\\|$q_char)+?)    (\2) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Euhc::filetest(qw($1),$2)"; }
    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1),$2)"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+)                                   /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest qw($1),";    }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+)                                         /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest(qw($1),$2)"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)                (\") /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)                (\#) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)               (\)) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)               (\}) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)             (\]) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)               (\>) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)                (\2) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\'|\\\\|$q_char)+?)       (\') /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)       (\#) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)      (\)) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)      (\}) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?)    (\]) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)      (\>) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\2|\\\\|$q_char)+?)       (\2) /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Euhc::$1($2)";             }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \)                          /oxgc) { $slash = 'm//'; return "Char::Euhc::$1($2)";             }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+)                                      /oxgc) { $slash = 'm//'; return "Char::Euhc::$1";                 }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+)                                            /oxgc) { $slash = 'm//'; return "Char::Euhc::$1(::"."$2)";        }
    elsif (/\G -(t)                            \s+ (\w+)                                            /oxgc) { $slash = 'm//'; return "-t $2";                     }
    elsif (/\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(])                                /oxgc) { $slash = 'm//'; return 'Char::Euhc::lstat';              }
    elsif (/\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(])                                /oxgc) { $slash = 'm//'; return 'Char::Euhc::stat';               }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Euhc::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Euhc::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::fc_';               }
    elsif (/\G \b lstat \b         (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::lstat_';            }
    elsif (/\G \b stat \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::stat_';             }
    elsif (/\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                     \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return "Char::Euhc::filetest_(qw($1))"; }
    elsif (/\G    -([rwxoRWXOezsfdlpSbcugkTBMAC])
                     \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return "Char::Euhc::${1}_";             }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::glob_';             }
    elsif (/\G \b reverse \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_reverse;          }
    elsif (/\G \b getc \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_getc;             }
    elsif (/\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Euhc::opendir$1*";        }
    elsif (/\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Euhc::opendir$1*";        }
    elsif (/\G \b unlink \b        (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Euhc::unlink';            }

# chdir
    elsif (/\G \b (chdir) \b    (?! \s* => ) /oxgc) {
        $slash = 'm//';

        my $e = 'Char::Euhc::chdir';
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Euhc::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Euhc::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Euhc::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Euhc::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Euhc::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Euhc::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Euhc::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Euhc::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Euhc::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Euhc::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Euhc::split' . $e . qq     {" "};  }
...
# do
    ...
    elsif (/\G \b do (?= \s+ (?: q | qq | qx ) \b)                                    /oxmsgc) { return 'Char::Euhc::do';         }
    elsif (/\G \b do (?= \s+ \w+)                                                     /oxmsgc) { return 'do';                }
    elsif (/\G \b do (?= \s* \$ \w+ (?: ::\w+)* \( )                                  /oxmsgc) { return 'do';                }
    elsif (/\G \b do \b                                                               /oxmsgc) { return 'Char::Euhc::do';         }
...
# require else
    elsif (/\G \b require                                   \s* ;                     /oxmsgc) { return 'Char::Euhc::require;';   }
    elsif (/\G \b require \b                                                          /oxmsgc) { return 'Char::Euhc::require';    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Char::Euhc::PREMATCH()]}
            ...
            $e_string .= q{Char::Euhc::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Char::Euhc::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Char::Euhc::MATCH()};
            $slash = 'div';
        }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> @{[Char::Euhc::POSTMATCH()]}
            ...
            $e_string .= q{Char::Euhc::POSTMATCH()};
            $slash = 'div';
        }
...
# subroutines of package Char::Euhc
        ...
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Euhc::chop';    $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::UHC::index \b                               /oxgc) { $e_string .= 'Char::UHC::index';    $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Euhc::index';   $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::UHC::rindex \b                              /oxgc) { $e_string .= 'Char::UHC::rindex';   $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Euhc::rindex';  $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Euhc::lc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Euhc::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Euhc::uc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Euhc::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Euhc::fc';      $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\2) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\'|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\2|\\\\|$q_char)+?)    (\2) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Euhc::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+)                                   /oxgc) { $e_string .= "Char::Euhc::filetest qw($1),";    $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+)                                         /oxgc) { $e_string .= "Char::Euhc::filetest(qw($1),$2)"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)                (\") /oxgc) { $e_string .= "Char::Euhc::$1(" . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)                (\#) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)               (\)) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)               (\}) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)             (\]) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)               (\>) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)                (\2) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\'|\\\\|$q_char)+?)       (\') /oxgc) { $e_string .= "Char::Euhc::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)       (\#) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)      (\)) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)      (\}) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?)    (\]) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)      (\>) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\2|\\\\|$q_char)+?)       (\2) /oxgc) { $e_string .= "Char::Euhc::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Euhc::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \)                          /oxgc) { $e_string .= "Char::Euhc::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+)                                      /oxgc) { $e_string .= "Char::Euhc::$1";          $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+)                                            /oxgc) { $e_string .= "Char::Euhc::$1(::"."$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G -(t)                            \s+ (\w+)                                            /oxgc) { $e_string .= "-t $2";              $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(])                                /oxgc) { $e_string .= 'Char::Euhc::lstat';       $slash = 'm//'; }
        elsif ($string =~ /\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(])                                /oxgc) { $e_string .= 'Char::Euhc::stat';        $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Euhc::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Euhc::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Euhc::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Euhc::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Euhc::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Euhc::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Euhc::fc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat \b                                           /oxgc) { $e_string .= 'Char::Euhc::lstat_';            $slash = 'm//'; }
        elsif ($string =~ /\G \b stat \b                                            /oxgc) { $e_string .= 'Char::Euhc::stat_';             $slash = 'm//'; }
        elsif ($string =~ /\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                 \b                 /oxgc) { $e_string .= "Char::Euhc::filetest_(qw($1))"; $slash = 'm//'; }
        elsif ($string =~ /\G    -([rwxoRWXOezsfdlpSbcugkTBMAC]) \b                 /oxgc) { $e_string .= "Char::Euhc::${1}_";             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Euhc::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Euhc::glob_';             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Euhc::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Euhc::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b unlink \b                                          /oxgc) { $e_string .= 'Char::Euhc::unlink';            $slash = 'm//'; }

# chdir
        elsif ($string =~ /\G \b (chdir) \b (?! \s* => ) /oxgc) {
            $slash = 'm//';

            $e_string .= 'Char::Euhc::chdir';
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Euhc::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Euhc::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Euhc::dot_s}';
        }
        else {
            return '${Char::Euhc::dot}';
        }
    }
    else {
        return Char::Euhc::classic_character_class($char);
    }
}
...
sub e_capture {
    ...
    return join '', '${Char::Euhc::capture(', $_[0], ')}';
    return join '', '$',                 $_[0];
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Euhc::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Euhc::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Euhc::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Euhc::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Euhc::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Euhc::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Euhc::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Euhc::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Euhc::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Euhc::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Euhc::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Euhc::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Euhc::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Euhc::uc($char[$i]) ne Char::Euhc::fc($char[$i]))) {
            if (CORE::length(Char::Euhc::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Euhc::uc($char[$i])       . Char::Euhc::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Euhc::uc($char[$i]) . '|' . Char::Euhc::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Euhc::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Euhc::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Euhc::ignorecase(Char::Euhc::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Euhc::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Euhc::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Euhc::ignorecase(Char::Euhc::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Euhc::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Euhc::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Euhc::ignorecase(Char::Euhc::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Euhc::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::UHC::re_t=%s; local $Char::UHC::re_a=''; while($Char::UHC::re_t =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s$Char::UHC::re_t="$Char::UHC::re_a${1}$Char::UHC::re_r$'"; pos($Char::UHC::re_t)=length "$Char::UHC::re_a${1}$Char::UHC::re_r"; $Char::UHC::re_a=substr($Char::UHC::re_t,0,pos($Char::UHC::re_t)); } return $Char::UHC::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::UHC::re_r=eval $Char::UHC::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::UHC::re_t=%s; while($Char::UHC::re_t =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s$Char::UHC::re_t="$`$Char::UHC::re_r$'"; pos($Char::UHC::re_t)=length "$`$Char::UHC::re_r"; } return $Char::UHC::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::UHC::re_r=%s; %s"%s$Char::UHC::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::UHC::re_n=0; local $Char::UHC::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="$Char::UHC::re_a${1}$Char::UHC::re_r$'"; pos(%s)=length "$Char::UHC::re_a${1}$Char::UHC::re_r"; $Char::UHC::re_a=substr(%s,0,pos(%s)); $Char::UHC::re_n++} return %s$Char::UHC::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::UHC::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="$`$Char::UHC::re_r$'"; pos(%s)=length "$`$Char::UHC::re_r"; $Char::UHC::re_n++} return %s$Char::UHC::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="%s$Char::UHC::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::UHC::re_r=%s; %s%s="%s$Char::UHC::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::UHC::re_r=eval $Char::UHC::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_chdir_q {
    ...
    if ($^W) {
        if (Char::Euhc::_MSWin32_5Cended_path($string)) {
            if ($] !~ /^5\.005/) {
                warn <<END;
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Euhc::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Euhc::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Euhc::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::UHC \s* ([^\x81-\xFE;]*) ; \s* \n? $/oxms) {
                ...
                return qq[BEGIN { Char::Euhc::require '$expr'; $module->import() if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no_noparam {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Euhc::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::UHC \s* ([^\x81-\xFE;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Euhc::require '$expr'; $module->unimport() if $module->can('unimport'); }];
            }
            last;
        }
    }
...
sub e_use {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Euhc::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::UHC \s* ([^\x81-\xFE;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Euhc::require '$expr'; $module->import($list) if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Euhc::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::UHC \s* ([^\x81-\xFE;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Euhc::require '$expr'; $module->unimport($list) if $module->can('unimport'); }];
            }
            last;
        }
    }
...
=item * You'll have to download Char/UHC.pm and Char/Euhc.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::UHC;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::UHC;
  use Char::UHC ver.sion;             --- require minimum version
  use Char::UHC ver.sion.0;           --- expects version (match or die)
  use Char::UHC qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::UHC ver.sion qw(ord reverse getc);
  use Char::UHC ver.sion.0 qw(ord reverse getc);

  # "no Char::UHC;" not supported

  or

  $ perl Char/UHC.pm UHC_script.pl > Escaped_script.pl.e
...
Char::UHC software is "middleware" between perl interpreter and your Perl script
written in UHC.
...
By "use Char::UHC;", it automatically interpret your script as UHC. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/UHC.pm, Char/Euhc.pm
  +---------------------------------------+
...
import of Char/Euhc.pm works well.
...
=head1 Software Composition
   ...
   Char/UHC.pm               --- source code filter to escape UHC
   Char/Euhc.pm              --- run-time routines for Char/UHC.pm
...
You need write 'use Char::UHC;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::UHC;
  ---------------------------------

=head1 Escaping Multiple-Octet Code (Char/UHC.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/UHC.pm provides)

Char/UHC.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Euhc::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (Char/UHC.pm provides)

Char/UHC.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char/UHC.pm provides)

Char/UHC.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Euhc::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Euhc::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Euhc::ignorecase()' (Char/UHC.pm provides)

Char/UHC.pm applies calling 'Char::Euhc::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Euhc::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Euhc::matched/
  m/regexp/               m/ditto$Char::Euhc::matched/
  ?regexp?                m?ditto$Char::Euhc::matched?
  m?regexp?               m?ditto$Char::Euhc::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Euhc::matched/) ?
  s/regexp/replacement/   eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Euhc::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Euhc::split(qr/regexp/)
  split(m/regexp/)        Char::Euhc::split(qr/regexp/)
  split(qr/regexp/)       Char::Euhc::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Euhc::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Euhc::matched/
  m/regexp/b              m/(?:regexp)$Char::Euhc::matched/
  ?regexp?b               m?regexp$Char::Euhc::matched?
  m?regexp?b              m?regexp$Char::Euhc::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Euhc::matched/) ?
  s/regexp/replacement/b  eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Euhc::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Euhc.pm provides)
...
Definitions in Char/Euhc.pm.
  ...
  ${Char::Euhc::eS}             qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\s])}
  ...
  ${Char::Euhc::not_space}      qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\s\x0B])}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/UHC.pm provides)

Char/UHC.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/UHC.pm and Char/Euhc.pm provide)

Insert 'Char::Euhc::' at head of function name. Char/Euhc.pm provides your script Char::Euhc::*
subroutines.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Euhc::split     Character
  tr///       Char::Euhc::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Euhc::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Euhc::chop      Character
  index       Char::Euhc::index     Character
  rindex      Char::Euhc::rindex    Character
  lc          Char::Euhc::lc        Character
  lcfirst     Char::Euhc::lcfirst   Character
  uc          Char::Euhc::uc        Character
  ucfirst     Char::Euhc::ucfirst   Character
  fc          Char::Euhc::fc        Character
  chr         Char::Euhc::chr       Character
  glob        Char::Euhc::glob      Character
  lstat       Char::Euhc::lstat     Character
  opendir     Char::Euhc::opendir   Character
  stat        Char::Euhc::stat      Character
  unlink      Char::Euhc::unlink    Character
  chdir       Char::Euhc::chdir     Character
  do          Char::Euhc::do        Character
  require     Char::Euhc::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Char::Euhc::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Char::Euhc::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Char::Euhc::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Char::Euhc::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Char::Euhc::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Char::Euhc::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Char/UHC.pm and Char/Euhc.pm provide)

Insert 'Char::Euhc::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Char::Euhc::r   File or directory is readable by this (effective) user or group
  -w       Char::Euhc::w   File or directory is writable by this (effective) user or group
  -e       Char::Euhc::e   File or directory name exists
  -x       Char::Euhc::x   File or directory is executable by this (effective) user or group
  -z       Char::Euhc::z   File exists and has zero size (always false for directories)
  -f       Char::Euhc::f   Entry is a plain file
  -d       Char::Euhc::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Euhc::T   File looks like a "text" file
  -B       Char::Euhc::B   File looks like a "binary" file
  -M       Char::Euhc::M   Modification age (measured in days)
  -A       Char::Euhc::A   Access age (measured in days)
  -C       Char::Euhc::C   Inode-modification age (measured in days)
  -s       Char::Euhc::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Euhc::R   File or directory is readable by this real user or group
  -W       Char::Euhc::W   File or directory is writable by this real user or group
  -X       Char::Euhc::X   File or directory is executable by this real user or group
  -l       Char::Euhc::l   Entry is a symbolic link
  -S       Char::Euhc::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Euhc::o   File or directory is owned by this (effective) user
  -O       Char::Euhc::O   File or directory is owned by this real user
  -p       Char::Euhc::p   Entry is a named pipe (a "fifo")
  -b       Char::Euhc::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Euhc::c   Entry is a character-special file (like an I/O device)
  -u       Char::Euhc::u   File or directory is setuid
  -g       Char::Euhc::g   File or directory is setgid
  -k       Char::Euhc::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Char::UHC::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        Char::UHC::ord
  reverse    Char::UHC::reverse
  getc       Char::UHC::getc
  length     Char::UHC::length
  substr     Char::UHC::substr
  index      Char::UHC::index          See 'About Indexes'
  rindex     Char::UHC::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Euhc::index)
  Char::UHC::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Euhc::rindex)
  Char::UHC::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  If you import ord "use Char::UHC qw(ord);", ord of your script will be rewritten in
  Char::UHC::ord. Char::UHC::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::UHC::reverse(@list);
  $reverse = Char::UHC::reverse(@list);
  ...
  If you import reverse "use Char::UHC qw(reverse);", reverse of your script will be
  rewritten in Char::UHC::reverse. Char::UHC::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  If you import getc "use Char::UHC qw(getc);", getc of your script will be rewritten
  in Char::UHC::getc. Char::UHC::getc is not compatible with getc of JPerl.

=item * Length by UHC Character

  $length = Char::UHC::length($string);
  $length = Char::UHC::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the Char::UHC::length
  of $_.

  Do not try to use Char::UHC::length to find the size of an array or hash. Use scalar
...
=item * Substr by UHC Character
  ...
  $substr = Char::UHC::substr($string,$offset,$length,$replacement);
  $substr = Char::UHC::substr($string,$offset,$length);
  $substr = Char::UHC::substr($string,$offset);
  ...
  my $color  = Char::UHC::substr $s, 4, 5;      # black
  my $middle = Char::UHC::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::UHC::substr $s, 14;        # climbed the green tree
  my $tail   = Char::UHC::substr $s, -4;        # tree
  my $z      = Char::UHC::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::UHC::substr() subroutine as an
  ...
  end of the string, Char::UHC::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::UHC::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::UHC::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::UHC::substr $name, 7;      # returns undef, with warning
  Char::UHC::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::UHC::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::UHC::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::UHC::substr() acts
...
=item * Index by UHC Character
  ...
  $index = Char::UHC::index($string,$substring,$offset);
  $index = Char::UHC::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::UHC::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by UHC Character
  ...
  $rindex = Char::UHC::rindex($string,$substring,$offset);
  $rindex = Char::UHC::rindex($string,$substring);

  This subroutine works just like Char::UHC::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::UHC::index).
  ...
  $pos = Char::UHC::length($string);
  while (($pos = Char::UHC::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Euhc::glob() subroutine. See Char::Euhc::glob
  of Char/Euhc.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::UHC qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::UHC qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Subroutines (Char/UHC.pm provides)

Char/UHC.pm removes 'bytes::' at head of subroutine name.
...
'use utf8;' to 'use Char::UHC;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Euhc.pm provides these
  functions.
...
=item * Char::UHC::substr as Lvalue

If Perl version is older than 5.14, Char::UHC::substr differs from CORE::substr, and
...
Char::UHC::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Euhc::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Char::Euhc::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Euhc::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Euhc::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Euhc::MATCH()       $1
  ${&}            Char::Euhc::MATCH()       $1
  $MATCH          Char::Euhc::MATCH()       $1
  ${^MATCH}       Char::Euhc::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Char::Euhc::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Euhc::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Euhc::*(),
Char::Euhc::lstat(), and Char::Euhc::stat() on Microsoft Windows open the file for the path
...
Char::UHC software remains one language and one interpreter.
