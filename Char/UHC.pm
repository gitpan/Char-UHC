...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Euhc::uc($char[$i]) ne Char::Euhc::lc($char[$i]))) {
            $char[$i] = '[' . Char::Euhc::uc($char[$i]) . Char::Euhc::lc($char[$i]) . ']';
        }
...
sub e_require {
    ...
    return qq<Char::Euhc::require '$expr';>;
}
...
http://search.cpan.org/dist/Char-Char::UHC/Char::UHC/JA.pm
...
=item * Length By UHC Character
  ...
  $string is omitted, it returns the Char::UHC::length of $_.
...
=item * Substr By UHC Character
  ...
  area past the end of the string, Char::UHC::substr raises an exception.
...
=item * Rindex By UHC Character
  ...
  $rindex = Char::UHC::rindex($string,$substring,$position);
  $rindex = Char::UHC::rindex($string,$substring);

  This function works just like Char::UHC::index except that it returns the position of
