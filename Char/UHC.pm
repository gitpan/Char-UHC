...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Euhc::function_ord     = 'Char::UHC::ord'     if /\A ord \z/oxms;
                    $Char::Euhc::function_ord_    = 'Char::UHC::ord_'    if /\A ord \z/oxms;
                    $Char::Euhc::function_reverse = 'Char::UHC::reverse' if /\A reverse \z/oxms;
                    $Char::Euhc::function_getc    = 'Char::UHC::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::UHC::escape();
...
=head1 Escaping Multiple-Octet Code (Char::UHC software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::UHC software provides)

Char::UHC software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (Char::UHC software provides)

Char::UHC software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::UHC software provides)

Char::UHC software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Euhc::ignorecase()' (Char::UHC software provides)

Char::UHC software applies calling 'Char::Euhc::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Euhc::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::UHC software provides)

Char::UHC software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::UHC software provide)
...
=head1 Escaping File Test Operators (Char::UHC software provide)
...
=item * Eval UHC Script
  ...
  $eval = Char::UHC::eval { block };
  $eval = Char::UHC::eval $expr;
  $eval = Char::UHC::eval;

  The Char::UHC::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::UHC::eval { block }
  and Char::UHC::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::UHC::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::UHC::eval, the value returned from an Char::UHC::eval is
  ...
  Char::UHC::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::UHC::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::UHC::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::UHC::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::UHC::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::UHC::eval cannot trap syntax errors.

  The Char::UHC::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::UHC::eval is complete, as do any subroutine or format definitions.
  The code of the Char::UHC::eval is treated as a block, so any locally scoped
  variables declared within the Char::UHC::eval last only until the Char::UHC::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::UHC::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::UHC::eval { block }    eval { block }
  Char::UHC::eval $expr        eval Char::UHC::escape $expr
  Char::UHC::eval              eval Char::UHC::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::UHC::eval does not exist. If it exists,
  you will troubled, when Char::UHC::eval has a parameter that is single quoted
  string included my variables. Char::UHC::escape is a subroutine that makes Perl
  ...
  Since Char::UHC::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::UHC::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::UHC::eval $expr and Char::UHC::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::UHC::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::UHC::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::UHC::eval '$answer =';   # sets $@
  ...
  Using the Char::UHC::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::UHC::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::UHC::eval { die "foo lives here" };
  ...
  With an Char::UHC::eval, you should be especially careful to remember what's being
  looked at when:

      Char::UHC::eval $x;        # CASE 1
      Char::UHC::eval "$x";      # CASE 2

      Char::UHC::eval '$x';      # CASE 3
      Char::UHC::eval { $x };    # CASE 4

      Char::UHC::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  The block of Char::UHC::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::UHC software provides)

Char::UHC software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::UHC software and
