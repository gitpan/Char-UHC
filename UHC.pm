...
######################################################################
#
# UHC - Source code filter to UHC script
#
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  m?regexp?               m?ditto@Euhc::matched?
  s/regexp/replacement/   ($_ =~ m/ditto@Euhc::matched/) ?
                          eval{ Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  m?regexp?b              m?regexp@Euhc::matched?
  s/regexp/replacement/b  ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)@Euhc::matched/) ?
                          eval{ Euhc::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
