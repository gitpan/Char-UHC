...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Euhc::uc($char[$i]) ne Euhc::lc($char[$i]))) {
            $char[$i] = '[' . Euhc::uc($char[$i]) . Euhc::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-UHC/UHC/JA.pm
...
Definitions in Euhc.pm.
  ...
  @{Euhc::eV}             qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x0C\x0A\x0D])}
