...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Euhc::binmode(FILEHANDLE, $disciplines);
  Euhc::binmode(FILEHANDLE);
  Euhc::binmode($filehandle, $disciplines);
  Euhc::binmode($filehandle);

  * two arguments

  If you are using perl5.005 other than MacPerl, UHC software emulate perl5.6's
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Euhc::open(FILEHANDLE, $mode, $expr);
  $rc = Euhc::open(FILEHANDLE, $expr);
  $rc = Euhc::open(FILEHANDLE);
  $rc = Euhc::open(my $filehandle, $mode, $expr);
  $rc = Euhc::open(my $filehandle, $expr);
  $rc = Euhc::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Euhc::open($fh, ">logfile")     # $fh is autovivified
  ...
  Euhc::open my $fh, ">logfile" or die ...
  ...
  Euhc::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Euhc::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Euhc::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Euhc::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Euhc::open(FH, "<:crlf:encoding(UHC)", $path) or die "can't open $path: $!";
